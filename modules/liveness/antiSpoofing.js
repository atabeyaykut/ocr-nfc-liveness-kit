...
class AntiSpoofingDetector {
  ...
  async checkSpoof(frameData) {
    try {
      ...
      Logger.error('Anti-spoofing check failed:', error.message);
      return {
        isReal: false,
        confidence: 0,
        reason: `Anti-spoofing hatası: ${error.message}`,
  ...
  _analyze3DDepth(face) {
    try {
      ...
      // Calculate depth variance from face contours
      let depthVariance = 0;
      let contourComplexity = 0;

      if (contours.face && contours.face.length > 0) {
        // Analyze face contour for 3D characteristics
        const facePoints = contours.face;
        const zVariations = this._calculateZVariations(facePoints);
        depthVariance = this._calculateVariance(zVariations);
        contourComplexity = this._calculateContourComplexity(facePoints);
      }

      // Analyze landmark depth relationships
      const landmarkDepth = this._analyzeLandmarkDepth(landmarks);
      ...
      return { score: 0.5, isReal: false, details: { error: error.message } };
    }
  }
  ...
  _analyzeEyeMovement(face) {
    try {
      ...
      const blinkConsistency = this._analyzeBlinkConsistency(leftEyeOpen, rightEyeOpen);
      
      // Check eye movement synchronization
      const eyeSync = Math.abs(leftEyeOpen - rightEyeOpen);
      const syncScore = 1.0 - Math.min(1.0, eyeSync / this.config.eyeMovementVariance);

      // Analyze natural eye movement patterns
      const naturalMovement = this._analyzeNaturalEyeMovement();

      const score = (blinkConsistency * 0.4 + syncScore * 0.3 + naturalMovement * 0.3);
  ...
  _analyzeTexture(frameData) {
    try {
      ...
      const score = Math.min(1.0,
        ...
        (1.0 - pixelationScore / this.config.pixelationDetectionThreshold) * 0.3 +
        (1.0 - reflectionScore / this.config.screenReflectionThreshold) * 0.2 +
      ...
      return { score: 0.6, isReal: true, details: { error: error.message } };
    }
  }
  ...
  _analyzeTemporalConsistency() {
    try {
      if (this.frameHistory.length < 3) {
        return { score: 0.5, isReal: true, details: { insufficientFrames: true } };
      }
      ...
      const score = (movementSmoothness * 0.4 + naturalVariation * 0.3 + qualityConsistency * 0.3);
      ...
      return { score: 0.5, isReal: true, details: { error: error.message } };
    }
  }
  ...
  _combineAnalyses(analyses) {
    ...
    const isReal = finalScore >= this.config.minConfidenceForReal;
    const confidence = Math.min(0.99, Math.max(0.01, finalScore));

    let reason = '';
    if (!isReal) {
      if (analyses.texture.score < 0.5) reason = 'Ekran/fotoğraf tespit edildi';
      else if (analyses.eyeMovement.score < 0.5) reason = 'Doğal göz hareketi algılanamadı';
      else if (analyses.depth.score < 0.5) reason = '3D yüz derinliği yetersiz';
      else if (analyses.temporal.score < 0.5) reason = 'Hareket tutarsızlığı tespit edildi';
  ...
  _analyzeBlinkConsistency(leftEyeOpen, rightEyeOpen) {
    ...
    this.movementHistory.push({ leftEyeOpen, rightEyeOpen, timestamp: Date.now() });
    ...
    for (let i = 1; i < this.movementHistory.length - 1; i++) {
      ...
      if (prev.leftEyeOpen > 0.7 && prev.rightEyeOpen > 0.7 &&
          curr.leftEyeOpen < 0.3 && curr.rightEyeOpen < 0.3 &&
          next.leftEyeOpen > 0.7 && next.rightEyeOpen > 0.7) {
        blinkCount++;
        
        // Check if blink duration is natural
        const duration = next.timestamp - prev.timestamp;
        if (duration >= this.config.blinkDurationMin && duration <= this.config.blinkDurationMax) {
          naturalBlinks++;
        }
      }
    }
  ...
  _analyzeNaturalEyeMovement() {
    ...
    const leftEyeVariance = this._calculateVariance(recentMovements.map(m => m.leftEyeOpen));
    const rightEyeVariance = this._calculateVariance(recentMovements.map(m => m.rightEyeOpen));
  ...
  _calculateMovementSmoothness() {
    ...
    for (let i = 1; i < this.frameHistory.length - 1; i++) {
      ...
      if (prev.bounds && curr.bounds && next.bounds) {
        ...
        const smoothnessRatio = Math.min(movement1, movement2) / Math.max(movement1, movement2);
  ...
  _calculateQualityConsistency() {
    ...
    const confidences = this.frameHistory.map(frame => frame.face.trackingId ? 1.0 : 0.5);
...
export const checkSpoof = async (frameData, config = {}) => {
  const detector = new AntiSpoofingDetector(config);
  return await detector.checkSpoof(frameData);
};

export { AntiSpoofingDetector, ANTI_SPOOFING_CONFIG };
export default AntiSpoofingDetector;
