...
public class QTESLASigner implements MessageSigner {
    ...
    public boolean verifySignature(byte[] bArr, byte[] bArr2) {
        ...
        if (securityCategory == 5) {
            r42 = QTesla1p.verifying(bArr, bArr2, 0, bArr2.length, this.publicKey.getPublicData());
        } else {
            if (securityCategory != 6) {
                throw new IllegalArgumentException("unknown security category: " + this.publicKey.getSecurityCategory());
            }
            r42 = QTesla3p.verifying(bArr, bArr2, 0, bArr2.length, this.publicKey.getPublicData());
        }
        return r42 == 0;
    }
}
