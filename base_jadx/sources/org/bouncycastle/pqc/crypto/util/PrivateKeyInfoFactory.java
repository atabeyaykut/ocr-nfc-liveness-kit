...
public class PrivateKeyInfoFactory {
    ...
    public static PrivateKeyInfo createPrivateKeyInfo(AsymmetricKeyParameter asymmetricKeyParameter, ASN1Set aSN1Set) throws IOException {
        ...
        if (asymmetricKeyParameter instanceof NHPrivateKeyParameters) {
            ...
            for (int r12 = 0; r12 != secData.length; r12++) {
                Pack.shortToLittleEndian(secData[r12], bArr, r12 * 2);
            }
    ...
    private static XMSSPrivateKey xmssCreateKeyStructure(XMSSPrivateKeyParameters xMSSPrivateKeyParameters) throws IOException {
        ...
        int r72 = (int) XMSSUtil.bytesToXBigEndian(encoded, 0, 4);
        if (!XMSSUtil.isIndexValid(height, r72)) {
            throw new IllegalArgumentException("index out of bounds");
        }
        byte[] bArrExtractBytesAtOffset = XMSSUtil.extractBytesAtOffset(encoded, 4, treeDigestSize);
        int r32 = 4 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset2 = XMSSUtil.extractBytesAtOffset(encoded, r32, treeDigestSize);
        int r33 = r32 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset3 = XMSSUtil.extractBytesAtOffset(encoded, r33, treeDigestSize);
        int r34 = r33 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset4 = XMSSUtil.extractBytesAtOffset(encoded, r34, treeDigestSize);
        int r35 = r34 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset5 = XMSSUtil.extractBytesAtOffset(encoded, r35, encoded.length - r35);
        try {
            BDS bds = (BDS) XMSSUtil.deserialize(bArrExtractBytesAtOffset5, BDS.class);
            return bds.getMaxIndex() != (1 << height) - 1 ? new XMSSPrivateKey(r72, bArrExtractBytesAtOffset, bArrExtractBytesAtOffset2, bArrExtractBytesAtOffset3, bArrExtractBytesAtOffset4, bArrExtractBytesAtOffset5, bds.getMaxIndex()) : new XMSSPrivateKey(r72, bArrExtractBytesAtOffset, bArrExtractBytesAtOffset2, bArrExtractBytesAtOffset3, bArrExtractBytesAtOffset4, bArrExtractBytesAtOffset5);
    ...
    private static XMSSMTPrivateKey xmssmtCreateKeyStructure(XMSSMTPrivateKeyParameters xMSSMTPrivateKeyParameters) throws IOException {
        ...
        int r32 = (height + 7) / 8;
        long jBytesToXBigEndian = (int) XMSSUtil.bytesToXBigEndian(encoded, 0, r32);
        if (!XMSSUtil.isIndexValid(height, jBytesToXBigEndian)) {
            throw new IllegalArgumentException("index out of bounds");
        }
        int r33 = r32 + 0;
        byte[] bArrExtractBytesAtOffset = XMSSUtil.extractBytesAtOffset(encoded, r33, treeDigestSize);
        int r34 = r33 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset2 = XMSSUtil.extractBytesAtOffset(encoded, r34, treeDigestSize);
        int r35 = r34 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset3 = XMSSUtil.extractBytesAtOffset(encoded, r35, treeDigestSize);
        int r36 = r35 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset4 = XMSSUtil.extractBytesAtOffset(encoded, r36, treeDigestSize);
        int r37 = r36 + treeDigestSize;
        byte[] bArrExtractBytesAtOffset5 = XMSSUtil.extractBytesAtOffset(encoded, r37, encoded.length - r37);
        try {
