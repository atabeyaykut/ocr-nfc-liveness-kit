...
public class XMSSMTSigner implements StateAwareMessageSigner {
    ...
    public byte[] generateSignature(byte[] bArr) {
        ...
        synchronized (xMSSMTPrivateKeyParameters) {
            ...
            try {
                ...
                for (int r6 = 1; r6 < this.params.getLayers(); r6++) {
                    XMSSNode root = bDSState.get(r6 - 1).getRoot();
                    int leafIndex2 = XMSSUtil.getLeafIndex(treeIndex, height);
                    treeIndex = XMSSUtil.getTreeIndex(treeIndex, height);
                    OTSHashAddress oTSHashAddress2 = (OTSHashAddress) new OTSHashAddress.Builder().withLayerAddress(r6).withTreeAddress(treeIndex).withOTSAddress(leafIndex2).build();
                    WOTSPlusSignature wOTSPlusSignatureWotsSign = wotsSign(root.getValue(), oTSHashAddress2);
                    if (bDSState.get(r6) == null || XMSSUtil.isNewBDSInitNeeded(index, height, r6)) {
                        bDSState.put(r6, new BDS(this.xmssParams, this.privateKey.getPublicSeed(), this.privateKey.getSecretKeySeed(), oTSHashAddress2));
                    }
                    xMSSMTSignatureBuild.getReducedSignatures().add(new XMSSReducedSignature.Builder(this.xmssParams).withWOTSPlusSignature(wOTSPlusSignatureWotsSign).withAuthPath(bDSState.get(r6).getAuthenticationPath()).build());
                }
    ...
    public boolean verifySignature(byte[] bArr, byte[] bArr2) {
        ...
        int r12 = 1;
        while (r12 < this.params.getLayers()) {
            XMSSReducedSignature xMSSReducedSignature = xMSSMTSignatureBuild.getReducedSignatures().get(r12);
            int leafIndex2 = XMSSUtil.getLeafIndex(treeIndex, height);
            long treeIndex2 = XMSSUtil.getTreeIndex(treeIndex, height);
            rootNodeFromSignature = XMSSVerifierUtil.getRootNodeFromSignature(this.wotsPlus, height, rootNodeFromSignature.getValue(), xMSSReducedSignature, (OTSHashAddress) new OTSHashAddress.Builder().withLayerAddress(r12).withTreeAddress(treeIndex2).withOTSAddress(leafIndex2).build(), leafIndex2);
            r12++;
