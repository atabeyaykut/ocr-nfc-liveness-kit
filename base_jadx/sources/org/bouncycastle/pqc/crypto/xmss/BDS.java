...
public final class BDS implements Serializable {
    ...
    private BDS(BDS bds, int r52, ASN1ObjectIdentifier aSN1ObjectIdentifier) {
    ...
    private BDS(WOTSPlus wOTSPlus, int r32, int r42, int r52) {
        ...
        if (r42 <= r32 && r42 >= 2) {
            ...
            if (r33 % 2 == 0) {
                ...
                for (int r43 = 0; r43 < r33; r43++) {
                    this.treeHashInstances.add(new BDSTreeHash(r43));
                }
    ...
    public BDS(XMSSParameters xMSSParameters, int r42, int r52) {
        this(xMSSParameters.getWOTSPlus(), xMSSParameters.getHeight(), xMSSParameters.getK(), r52);
        this.maxIndex = r42;
        this.index = r52;
        this.used = true;
    }
    ...
    public BDS(XMSSParameters xMSSParameters, byte[] bArr, byte[] bArr2, OTSHashAddress oTSHashAddress, int r92) {
        this(xMSSParameters.getWOTSPlus(), xMSSParameters.getHeight(), xMSSParameters.getK(), (1 << xMSSParameters.getHeight()) - 1);
        initialize(bArr, bArr2, oTSHashAddress);
        while (this.index < r92) {
            nextAuthenticationPath(bArr, bArr2, oTSHashAddress);
            this.used = false;
        }
    }
    ...
    private void initialize(byte[] bArr, byte[] bArr2, OTSHashAddress oTSHashAddress) {
        ...
        for (int r22 = 0; r22 < (1 << this.treeHeight); r22++) {
            oTSHashAddress = (OTSHashAddress) new OTSHashAddress.Builder().withLayerAddress(oTSHashAddress.getLayerAddress()).withTreeAddress(oTSHashAddress.getTreeAddress()).withOTSAddress(r22).withChainAddress(oTSHashAddress.getChainAddress()).withHashAddress(oTSHashAddress.getHashAddress()).withKeyAndMask(oTSHashAddress.getKeyAndMask()).build();
            ...
            lTreeAddress = (LTreeAddress) new LTreeAddress.Builder().withLayerAddress(lTreeAddress.getLayerAddress()).withTreeAddress(lTreeAddress.getTreeAddress()).withLTreeAddress(r22).withTreeHeight(lTreeAddress.getTreeHeight()).withTreeIndex(lTreeAddress.getTreeIndex()).withKeyAndMask(lTreeAddress.getKeyAndMask()).build();
            XMSSNode xMSSNodeLTree = XMSSNodeUtil.lTree(this.wotsPlus, publicKey, lTreeAddress);
            hashTreeAddress = (HashTreeAddress) new HashTreeAddress.Builder().withLayerAddress(hashTreeAddress.getLayerAddress()).withTreeAddress(hashTreeAddress.getTreeAddress()).withTreeIndex(r22).withKeyAndMask(hashTreeAddress.getKeyAndMask()).build();
            while (!this.stack.isEmpty() && this.stack.peek().getHeight() == xMSSNodeLTree.getHeight()) {
                int height = r22 / (1 << xMSSNodeLTree.getHeight());
                if (height == 1) {
                    this.authenticationPath.add(xMSSNodeLTree);
                }
                if (height == 3 && xMSSNodeLTree.getHeight() < this.treeHeight - this.f11973k) {
                    this.treeHashInstances.get(xMSSNodeLTree.getHeight()).setNode(xMSSNodeLTree);
                }
                if (height >= 3 && (height & 1) == 1 && xMSSNodeLTree.getHeight() >= this.treeHeight - this.f11973k && xMSSNodeLTree.getHeight() <= this.treeHeight - 2) {
    ...
    private void nextAuthenticationPath(byte[] bArr, byte[] bArr2, OTSHashAddress oTSHashAddress) {
        ...
        int r02 = this.index;
        if (r02 > this.maxIndex - 1) {
            throw new IllegalStateException("index out of bounds");
        }
        int r03 = XMSSUtil.calculateTau(r02, this.treeHeight);
        if (((this.index >> (r03 + 1)) & 1) == 0 && r03 < this.treeHeight - 1) {
            this.keep.put(Integer.valueOf(r03), this.authenticationPath.get(r03));
        }
        LTreeAddress lTreeAddress = (LTreeAddress) new LTreeAddress.Builder().withLayerAddress(oTSHashAddress.getLayerAddress()).withTreeAddress(oTSHashAddress.getTreeAddress()).build();
        HashTreeAddress hashTreeAddress = (HashTreeAddress) new HashTreeAddress.Builder().withLayerAddress(oTSHashAddress.getLayerAddress()).withTreeAddress(oTSHashAddress.getTreeAddress()).build();
        if (r03 == 0) {
            ...
            int r52 = r03 - 1;
            HashTreeAddress hashTreeAddress2 = (HashTreeAddress) new HashTreeAddress.Builder().withLayerAddress(hashTreeAddress.getLayerAddress()).withTreeAddress(hashTreeAddress.getTreeAddress()).withTreeHeight(r52).withTreeIndex(this.index >> r03).withKeyAndMask(hashTreeAddress.getKeyAndMask()).build();
            WOTSPlus wOTSPlus2 = this.wotsPlus;
            wOTSPlus2.importKeys(wOTSPlus2.getWOTSPlusSecretKey(bArr2, oTSHashAddress), bArr);
            XMSSNode xMSSNodeRandomizeHash = XMSSNodeUtil.randomizeHash(this.wotsPlus, this.authenticationPath.get(r52), this.keep.get(Integer.valueOf(r52)), hashTreeAddress2);
            this.authenticationPath.set(r03, new XMSSNode(xMSSNodeRandomizeHash.getHeight() + 1, xMSSNodeRandomizeHash.getValue()));
            this.keep.remove(Integer.valueOf(r52));
            for (int r12 = 0; r12 < r03; r12++) {
                if (r12 < this.treeHeight - this.f11973k) {
                    list = this.authenticationPath;
                    xMSSNodeRemoveFirst = this.treeHashInstances.get(r12).getTailNode();
                } else {
                    list = this.authenticationPath;
                    xMSSNodeRemoveFirst = this.retain.get(Integer.valueOf(r12)).removeFirst();
                }
                list.set(r12, xMSSNodeRemoveFirst);
            }
            int r04 = Math.min(r03, this.treeHeight - this.f11973k);
            for (int r13 = 0; r13 < r04; r13++) {
                int r53 = ((1 << r13) * 3) + this.index + 1;
                if (r53 < (1 << this.treeHeight)) {
                    this.treeHashInstances.get(r13).initialize(r53);
                }
            }
        }
        for (int r42 = 0; r42 < ((this.treeHeight - this.f11973k) >> 1); r42++) {
    ...
    private void readObject(ObjectInputStream objectInputStream) throws ClassNotFoundException, IOException {
        ...
        int r02 = this.maxIndex;
        if (r02 > (1 << this.treeHeight) - 1 || this.index > r02 + 1 || objectInputStream.available() != 0) {
    ...
    public BDS withMaxIndex(int r22, ASN1ObjectIdentifier aSN1ObjectIdentifier) {
        return new BDS(this, r22, aSN1ObjectIdentifier);
    }
