...
public class ECMQVBasicAgreement implements BasicAgreement {
    ...
    private ECPoint calculateMqvAgreement(ECDomainParameters eCDomainParameters, ECPrivateKeyParameters eCPrivateKeyParameters, ECPrivateKeyParameters eCPrivateKeyParameters2, ECPublicKeyParameters eCPublicKeyParameters, ECPublicKeyParameters eCPublicKeyParameters2, ECPublicKeyParameters eCPublicKeyParameters3) {
        ...
        int r12 = (n10.bitLength() + 1) / 2;
        BigInteger bigIntegerShiftLeft = ECConstants.ONE.shiftLeft(r12);
        ...
        BigInteger bigIntegerMod = eCPrivateKeyParameters.getD().multiply(eCPointCleanPoint.getAffineXCoord().toBigInteger().mod(bigIntegerShiftLeft).setBit(r12)).add(eCPrivateKeyParameters2.getD()).mod(n10);
        BigInteger bit = eCPointCleanPoint3.getAffineXCoord().toBigInteger().mod(bigIntegerShiftLeft).setBit(r12);
        BigInteger bigIntegerMod2 = eCDomainParameters.getH().multiply(bigIntegerMod).mod(n10);
        return ECAlgorithms.sumOfTwoMultiplies(eCPointCleanPoint2, bit.multiply(bigIntegerMod2).mod(n10), eCPointCleanPoint3, bigIntegerMod2);
    }
