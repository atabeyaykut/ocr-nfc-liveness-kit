...
public class HPKE {
    ...
    public HPKE(byte b10, short s7, short s10, short s11) {
        this.mode = b10;
        this.kemId = s7;
        this.kdfId = s10;
        this.aeadId = s11;
        this.hkdf = new HKDF(s10);
        this.dhkem = new DHKEM(s7);
        this.Nk = s11 == 1 ? (short) 16 : (short) 32;
    }

    private void VerifyPSKInputs(byte b10, byte[] bArr, byte[] bArr2) {
        boolean z10 = !Arrays.areEqual(bArr, this.default_psk);
        if (z10 != (!Arrays.areEqual(bArr2, this.default_psk_id))) {
    ...
    private HPKEContext keySchedule(byte b10, byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4) throws DataLengthException, IllegalArgumentException {
    ...
    public byte[] receiveExport(byte[] bArr, AsymmetricCipherKeyPair asymmetricCipherKeyPair, byte[] bArr2, byte[] bArr3, int r12, byte[] bArr4, byte[] bArr5, AsymmetricKeyParameter asymmetricKeyParameter) {
        HPKEContext hPKEContext;
        byte b10 = this.mode;
        if (b10 == 0) {
            hPKEContext = setupBaseR(bArr, asymmetricCipherKeyPair, bArr2);
        ...
        return hPKEContext.export(bArr3, r12);
    }
    ...
    public byte[][] sendExport(AsymmetricKeyParameter asymmetricKeyParameter, byte[] bArr, byte[] bArr2, int r13, byte[] bArr3, byte[] bArr4, AsymmetricCipherKeyPair asymmetricCipherKeyPair) {
        HPKEContextWithEncapsulation hPKEContextWithEncapsulationSetupPSKS;
        ...
        bArr5[1] = hPKEContextWithEncapsulationSetupPSKS.export(bArr2, r13);
        return bArr5;
    }
