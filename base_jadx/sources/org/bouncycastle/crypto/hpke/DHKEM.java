...
class DHKEM {
    ...
    private boolean ValidateSk(BigInteger bigInteger) {
        BigInteger n10 = this.domainParams.getN();
        return bigInteger.compareTo(BigInteger.valueOf(1L)) >= 0 && bigInteger.compareTo(n10) < 0 && WNafUtil.getNafWeight(bigInteger) >= (n10.bitLength() >>> 2);
    }

    private byte[] formatBigIntegerBytes(byte[] bArr, int r52) {
        byte[] bArr2 = new byte[r52];
        if (bArr.length <= r52) {
            System.arraycopy(bArr, 0, bArr2, r52 - bArr.length, bArr.length);
        } else {
            System.arraycopy(bArr, bArr.length - r52, bArr2, 0, r52);
        }
        return bArr2;
    }
    ...
    public AsymmetricCipherKeyPair DeriveKeyPair(byte[] bArr) throws DataLengthException, IllegalArgumentException {
        ...
        switch (s7) {
            ...
            case 18:
                ...
                for (int r10 = 0; r10 <= 255; r10++) {
                    bArr2[0] = (byte) r10;
                    byte[] bArrLabeledExpand = this.hkdf.LabeledExpand(bArrLabeledExtract, bArrConcatenate, "candidate", bArr2, this.Nsk);
                    ...
                    if (ValidateSk(bigInteger)) {
                        ECPoint eCPointMultiply = new FixedPointCombMultiplier().multiply(this.domainParams.getG(), bigInteger);
                        return new AsymmetricCipherKeyPair((AsymmetricKeyParameter) new ECPublicKeyParameters(eCPointMultiply, this.domainParams), (AsymmetricKeyParameter) new ECPrivateKeyParameters(bigInteger, this.domainParams));
                    }
                }
