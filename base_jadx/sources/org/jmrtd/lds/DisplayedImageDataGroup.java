...
public abstract class DisplayedImageDataGroup extends DataGroup {
    ...
    public DisplayedImageDataGroup(int r12, InputStream inputStream) throws IOException {
    ...
    public DisplayedImageDataGroup(int r12, List<DisplayedImageInfo> list, int r32) {
        super(r12);
        if (list == null) {
            throw new IllegalArgumentException("imageInfos cannot be null");
        }
        this.displayedImageTagToUse = r32;
    ...
    public void readContent(InputStream inputStream) throws IOException {
        ...
        if (tag != 2) {
            throw new IllegalArgumentException(b.c(tag, new StringBuilder("Expected tag 0x02 in displayed image structure, found ")));
        }
        if (tLVInputStream.readLength() != 1) {
            throw new IllegalArgumentException("DISPLAYED_IMAGE_COUNT should have length 1");
        }
        int r02 = tLVInputStream.readValue()[0] & 255;
        for (int r12 = 0; r12 < r02; r12++) {
            DisplayedImageInfo displayedImageInfo = new DisplayedImageInfo(tLVInputStream);
            int displayedImageTag = displayedImageInfo.getDisplayedImageTag();
            if (r12 == 0) {
                this.displayedImageTagToUse = displayedImageTag;
            } else if (displayedImageTag != this.displayedImageTagToUse) {
