...
final class MultiDexExtractor implements Closeable {
    ...
    private static void extract(ZipFile zipFile, ZipEntry zipEntry, File file, String str) throws IOException {
        ...
        File fileCreateTempFile = File.createTempFile(b.e("tmp-", str), EXTRACTED_SUFFIX, file.getParentFile());
            ...
            try {
                ...
                while (true) {
                    ...
                    if (r22 == -1) {
                        ...
                        zipOutputStream.write(bArr, 0, r22);
                    }
                }
    ...
    private List<ExtractedDex> loadExistingExtractions(Context context, String str) throws IOException {
        ...
        SharedPreferences multiDexPreferences = getMultiDexPreferences(context);
        int r42 = multiDexPreferences.getInt(str + KEY_DEX_NUMBER, 1);
        ArrayList arrayList = new ArrayList(r42 + (-1));
        int r6 = 2;
        while (r6 <= r42) {
            ExtractedDex extractedDex = new ExtractedDex(this.dexDir, str2 + r6 + EXTRACTED_SUFFIX);
            if (!extractedDex.isFile()) {
                throw new IOException("Missing extracted secondary dex file '" + extractedDex.getPath() + "'");
            }
            extractedDex.crc = getZipCrc(extractedDex);
            long j10 = multiDexPreferences.getLong(str + KEY_DEX_CRC + r6, -1L);
            long j11 = multiDexPreferences.getLong(str + KEY_DEX_TIME + r6, -1L);
    ...
    private List<ExtractedDex> performExtractions() throws IOException {
        ...
        try {
            ...
            int r6 = 2;
            while (entry != null) {
                ExtractedDex extractedDex = new ExtractedDex(this.dexDir, str + r6 + EXTRACTED_SUFFIX);
                arrayList.add(extractedDex);
                ...
                if (!z10) {
                    throw new IOException("Could not create zip file " + extractedDex.getAbsolutePath() + " for secondary dex (" + r6 + ")");
                }
                r6++;
                entry = zipFile.getEntry(DEX_PREFIX + r6 + DEX_SUFFIX);
            }
            return arrayList;
        } finally {
            try {
    ...
    private static void putStoredApkInfo(Context context, String str, long j10, long j11, List<ExtractedDex> list) {
        ...
        editorEdit.putLong(c.h(new StringBuilder(), str, KEY_CRC), j11);
        editorEdit.putInt(str + KEY_DEX_NUMBER, list.size() + 1);
        int r52 = 2;
        for (ExtractedDex extractedDex : list) {
            editorEdit.putLong(str + KEY_DEX_CRC + r52, extractedDex.crc);
            editorEdit.putLong(str + KEY_DEX_TIME + r52, extractedDex.lastModified());
            r52++;
        }
        editorEdit.commit();
    }
    ...
    public List<? extends File> load(Context context, String str, boolean z10) throws IOException {
        ...
        Log.i(TAG, z10 ? "Forced extraction must be performed." : "Detected that extraction must be performed.");
