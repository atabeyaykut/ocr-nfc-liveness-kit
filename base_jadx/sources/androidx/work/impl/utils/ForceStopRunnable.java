...
public class ForceStopRunnable implements Runnable {
    ...
    private static PendingIntent getPendingIntent(Context context, int r32) {
        return PendingIntent.getBroadcast(context, -1, getIntent(context), r32);
    }
        ...
        try {
            ...
            if (z10) {
                for (WorkSpec workSpec : runningWork) {
                    workSpecDao.setState(WorkInfo.State.ENQUEUED, workSpec.f999id);
                    workSpecDao.markWorkSpecScheduled(workSpec.f999id, -1L);
                }
            }
    ...
    public boolean isForceStopped() {
        try {
            PendingIntent pendingIntent = getPendingIntent(this.mContext, BuildCompat.isAtLeastS() ? 570425344 : PKIFailureInfo.duplicateCertReq);
            if (Build.VERSION.SDK_INT >= 30) {
                ...
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    for (int r32 = 0; r32 < historicalProcessExitReasons.size(); r32++) {
                        if (((ApplicationExitInfo) historicalProcessExitReasons.get(r32)).getReason() == 10) {
                            return true;
                        }
                    }
                }
            ...
            if (multiProcessChecks()) {
                while (true) {
                    ...
                    try {
                        ...
                        r12 = this.mRetryCount + 1;
                        this.mRetryCount = r12;
                        if (r12 >= 3) {
                            ...
                            Logger.get().debug(TAG, String.format("Retrying after %s", Long.valueOf(r12 * BACKOFF_DURATION_MS)), e10);
