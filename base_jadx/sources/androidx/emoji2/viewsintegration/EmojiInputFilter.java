...
final class EmojiInputFilter implements InputFilter {
    ...
    public static void updateSelection(Spannable spannable, int r12, int r22) {
        if (r12 >= 0 && r22 >= 0) {
            Selection.setSelection(spannable, r12, r22);
        } else if (r12 >= 0) {
            Selection.setSelection(spannable, r12);
        } else if (r22 >= 0) {
            Selection.setSelection(spannable, r22);
        }
    }

    @Override // android.text.InputFilter
    public CharSequence filter(CharSequence charSequence, int r42, int r52, Spanned spanned, int r72, int r82) {
        ...
        if (loadState != 0) {
            boolean z10 = true;
            if (loadState == 1) {
                if (r82 == 0 && r72 == 0 && spanned.length() == 0 && charSequence == this.mTextView.getText()) {
                    z10 = false;
                }
                if (!z10 || charSequence == null) {
                    return charSequence;
                }
                if (r42 != 0 || r52 != charSequence.length()) {
                    charSequence = charSequence.subSequence(r42, r52);
                }
