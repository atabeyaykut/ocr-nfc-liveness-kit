...
public final class PackageInfoCompat {
    ...
    public static class Api28Impl {
        ...
        @DoNotInline
        public static boolean hasSigningCertificate(@NonNull PackageManager packageManager, @NonNull String str, @NonNull byte[] bArr, int r32) {
            return packageManager.hasSigningCertificate(str, bArr, r32);
        }
    }
    ...
    public static boolean hasSignatures(@NonNull PackageManager packageManager, @NonNull String str, @NonNull @Size(min = 1) Map<byte[], Integer> map, boolean z10) throws PackageManager.NameNotFoundException {
        ...
        for (byte[] bArr2 : setKeySet) {
            if (bArr2 == null) {
                throw new IllegalArgumentException(androidx.browser.browseractions.b.e("Cert byte array cannot be null when verifying ", str));
            }
            Integer num = map.get(bArr2);
            if (num == null) {
                throw new IllegalArgumentException(androidx.browser.browseractions.b.e("Type must be specified for cert when verifying ", str));
            }
            int r6 = num.intValue();
            if (r6 != 0 && r6 != 1) {
        ...
        if (signatures.size() != 0 && map.size() <= signatures.size() && (!z10 || map.size() == signatures.size())) {
            if (map.containsValue(1)) {
                ...
                for (int r82 = 0; r82 < signatures.size(); r82++) {
                    bArr[r82] = computeSHA256Digest(signatures.get(r82).toByteArray());
                }
            } else {
                bArr = null;
            }
