...
public final class BrowserServiceFileProvider extends FileProvider {
    ...
    private static Uri generateUri(Context context, String str) {
        return new Uri.Builder().scheme(CONTENT_SCHEME).authority(context.getPackageName() + AUTHORITY_SUFFIX).path(a.a(FILE_SUB_DIR_NAME, str, FILE_EXTENSION)).build();
    }
    ...
    public static void grantReadPermission(@NonNull Intent intent, @Nullable List<Uri> list, @NonNull Context context) {
        ...
        for (int r02 = 1; r02 < list.size(); r02++) {
            clipDataNewUri.addItem(new ClipData.Item(list.get(r02)));
        }
        intent.setClipData(clipDataNewUri);
    }

    @NonNull
    public static m5.a<Bitmap> loadBitmap(@NonNull final ContentResolver contentResolver, @NonNull final Uri uri) {
    ...
    @NonNull
    @UiThread
    public static ResolvableFuture<Uri> saveBitmap(@NonNull Context context, @NonNull Bitmap bitmap, @NonNull String str, int r92) {
        StringBuilder sbI = b.i(str, "_");
        sbI.append(Integer.toString(r92));
