...
class ClockFaceView extends RadialViewGroup implements ClockHandView.OnRotateListener {
    ...
    public ClockFaceView(@NonNull Context context, @Nullable AttributeSet attributeSet, int r10) {
        super(context, attributeSet, r10);
        this.textViewRect = new Rect();
        this.scratch = new RectF();
        this.textViewPool = new SparseArray<>();
        this.gradientPositions = new float[]{0.0f, 0.9f, 1.0f};
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, R.styleable.ClockFaceView, r10, R.style.Widget_MaterialComponents_TimePicker_Clock);
        ...
        this.valueAccessibilityDelegate = new AccessibilityDelegateCompat() { // from class: com.google.android.material.timepicker.ClockFaceView.2
            ...
            public void onInitializeAccessibilityNodeInfo(View view, @NonNull AccessibilityNodeInfoCompat accessibilityNodeInfoCompat) {
                ...
                int r32 = ((Integer) view.getTag(R.id.material_value_index)).intValue();
                if (r32 > 0) {
                    accessibilityNodeInfoCompat.setTraversalAfter((View) ClockFaceView.this.textViewPool.get(r32 - 1));
                }
                accessibilityNodeInfoCompat.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(0, 1, r32, 1, false, view.isSelected()));
            }
        };
    ...
    private void findIntersectingTextView() {
        ...
        for (int r12 = 0; r12 < this.textViewPool.size(); r12++) {
            TextView textView = this.textViewPool.get(r12);
            if (textView != null) {
    ...
    private static float max3(float f, float f10, float f11) {
        return Math.max(Math.max(f, f10), f11);
    }

    private void updateTextViews(@StringRes int r92) {
        LayoutInflater layoutInflaterFrom = LayoutInflater.from(getContext());
        int size = this.textViewPool.size();
        for (int r32 = 0; r32 < Math.max(this.values.length, size); r32++) {
            TextView textView = this.textViewPool.get(r32);
            if (r32 >= this.values.length) {
                removeView(textView);
                this.textViewPool.remove(r32);
            } else {
                if (textView == null) {
                    textView = (TextView) layoutInflaterFrom.inflate(R.layout.material_clockface_textview, (ViewGroup) this, false);
                    this.textViewPool.put(r32, textView);
                    addView(textView);
                }
                textView.setVisibility(0);
                textView.setText(this.values[r32]);
                textView.setTag(R.id.material_value_index, Integer.valueOf(r32));
                ViewCompat.setAccessibilityDelegate(textView, this.valueAccessibilityDelegate);
                textView.setTextColor(this.textColor);
                if (r92 != 0) {
                    textView.setContentDescription(getResources().getString(r92, this.values[r32]));
                }
            }
        }
    }
    ...
    public void onMeasure(int r32, int r42) {
        ...
        int r33 = (int) (this.clockSize / max3(this.minimumHeight / displayMetrics.heightPixels, this.minimumWidth / displayMetrics.widthPixels, 1.0f));
        int r43 = View.MeasureSpec.makeMeasureSpec(r33, 1073741824);
        setMeasuredDimension(r33, r33);
        super.onMeasure(r43, r43);
    }
    ...
    public void setValues(String[] strArr, @StringRes int r22) {
        this.values = strArr;
        updateTextViews(r22);
    }
}
