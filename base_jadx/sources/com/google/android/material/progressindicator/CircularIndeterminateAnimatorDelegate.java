...
final class CircularIndeterminateAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private void maybeUpdateSegmentColors(int r6) {
        for (int r12 = 0; r12 < 4; r12++) {
            float fractionInRange = getFractionInRange(r6, DELAY_TO_FADE_IN_MS[r12], 333);
            if (fractionInRange >= 0.0f && fractionInRange <= 1.0f) {
                int r13 = r12 + this.indicatorColorIndexOffset;
                int[] r62 = this.baseSpec.indicatorColors;
                int length = r13 % r62.length;
                int length2 = (length + 1) % r62.length;
                int r63 = MaterialColors.compositeARGBWithAlpha(r62[length], this.drawable.getAlpha());
                int r14 = MaterialColors.compositeARGBWithAlpha(this.baseSpec.indicatorColors[length2], this.drawable.getAlpha());
                this.segmentColors[0] = ArgbEvaluatorCompat.getInstance().evaluate(this.interpolator.getInterpolation(fractionInRange), Integer.valueOf(r63), Integer.valueOf(r14)).intValue();
                return;
            }
        }
    }
    ...
    private void updateSegmentPositions(int r92) {
        float[] fArr = this.segmentPositions;
        float f = this.animationFraction;
        fArr[0] = (f * 1520.0f) - 20.0f;
        fArr[1] = f * 1520.0f;
        for (int r02 = 0; r02 < 4; r02++) {
            float fractionInRange = getFractionInRange(r92, DELAY_TO_EXPAND_IN_MS[r02], 667);
            float[] fArr2 = this.segmentPositions;
            fArr2[1] = (this.interpolator.getInterpolation(fractionInRange) * 250.0f) + fArr2[1];
            float fractionInRange2 = getFractionInRange(r92, DELAY_TO_COLLAPSE_IN_MS[r02], 667);
        ...
        float f10 = fArr4[0];
        float f11 = fArr4[1];
        float f12 = ((f11 - f10) * this.completeEndFraction) + f10;
        fArr4[0] = f12;
        fArr4[0] = f12 / 360.0f;
        fArr4[1] = f11 / 360.0f;
    }
    ...
    public void setAnimationFraction(float f) {
        ...
        int r22 = (int) (f * 5400.0f);
        updateSegmentPositions(r22);
        maybeUpdateSegmentColors(r22);
