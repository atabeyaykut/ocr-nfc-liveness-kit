...
import { checkSpoof, AntiSpoofingDetector, ANTI_SPOOFING_CONFIG } from '../modules/liveness/antiSpoofing';
import { validateRealTimeResponse } from '../modules/liveness/validator';
...
describe('Anti-Spoofing Detection - Day 10', () => {
  ...
  describe('AntiSpoofingDetector Initialization', () => {
    test('should initialize with default configuration', () => {
      ...
      expect(newDetector.config).toEqual(expect.objectContaining(ANTI_SPOOFING_CONFIG));
    ...
    test('should validate blink + head turn combination', async () => {
      ...
      const combinedMovements = [
        ...
        { 
          ...
          leftEye: 0.3, rightEye: 0.3 // Blink while moving
        },
        { 
  ...
  describe('Integration with Validator', () => {
    test('should integrate anti-spoofing with real-time validation', async () => {
      const realFaceData = {
      ...
      expect(result.detectionType).toBe('real_time_face_detection_with_anti_spoofing');
    });

    test('should fail validation when spoofing detected', async () => {
      const fakeFaceData = {
        realTimeDetection: true,
        frameData: {
          faces: [{
            bounds: { x: 100, y: 100, width: 200, height: 200 },
            landmarks: {
              leftEye: { openProbability: 0.2, position: { x: 150, y: 140, z: 0 } },
              rightEye: { openProbability: 0.2, position: { x: 250, y: 140, z: 0 } },
    ...
    test('should handle anti-spoofing disabled mode', async () => {
      const faceData = {
        realTimeDetection: true,
        frameData: {
          faces: [{ /* fake data */ }]
  ...
  describe('Error Handling', () => {
    ...
    test('should handle detector reset properly', () => {
      ...
      detector.movementHistory = [{ leftEyeOpen: 0.8, rightEyeOpen: 0.8, timestamp: Date.now() }];
