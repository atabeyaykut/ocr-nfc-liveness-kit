...
describe("NFC Final Integration Tests - Stable Version", () => {
  ...
  afterEach(async () => {
    // Skip cleanup to avoid affecting subsequent tests
  });
  ...
  describe("🔧 Bug Fixes Verification", () => {
    ...
    test("should prevent duplicate readings effectively", async () => {
      ...
      await expect(nfcReader.readNFCData({ useRealNFC: true })).rejects.toThrow(
        /NFC reading already in progress/
      );
  ...
  describe("🔄 Lifecycle Management", () => {
    test("should handle complete NFC lifecycle correctly", async () => {
      ...
      // Test the core lifecycle functionality
      // The readNFCData method includes auto-initialization, so we can test the complete flow
      const result = await nfcReader.readNFCData({ useRealNFC: true });
      
      // Verify the data was read correctly
      expect(result.firstName).toBe("LIFECYCLE");
      expect(result.lastName).toBe("TEST");
      expect(result.idNumber).toBe("55555555555");
    ...
    test("should handle error recovery properly", async () => {
      ...
      // Simulate error
      NfcManager.requestTechnology.mockRejectedValueOnce(
