...
describe('NFCReader', () => {
  ...
  describe('NFC Support Detection', () => {
    test('should detect NFC not supported and call onError', async () => {
      ...
      expect(mockOnStatusChange).toHaveBeenCalledWith(NFC_STATUS.ERROR, NFC_STATUS.INITIALIZING);
    });
    ...
    test('should successfully initialize NFC when supported and enabled', async () => {
      ...
      expect(mockOnStatusChange).toHaveBeenCalledWith(NFC_STATUS.READY, NFC_STATUS.INITIALIZING);
    });
  });
  ...
  describe('Mock Data Reading', () => {
    ...
    test('should read mock NFC data successfully', async () => {
      ...
      expect(mockOnProgress).toHaveBeenCalledWith('Lütfen kimliğinizi telefonun arkasına yaklaştırın.');
    ...
    test('should update status during reading process', async () => {
      ...
      expect(mockOnStatusChange).toHaveBeenCalledWith(NFC_STATUS.SCANNING, NFC_STATUS.READY);
  ...
  describe('Callback Integration', () => {
    test('should call onStatusChange callback when status changes', async () => {
      ...
      expect(mockOnStatusChange).toHaveBeenCalledWith(NFC_STATUS.INITIALIZING, NFC_STATUS.IDLE);
      expect(mockOnStatusChange).toHaveBeenCalledWith(NFC_STATUS.READY, NFC_STATUS.INITIALIZING);
    });
    ...
    test('should call onProgress callback during reading', async () => {
      ...
      expect(mockOnProgress).toHaveBeenCalledWith('Lütfen kimliğinizi telefonun arkasına yaklaştırın.');
      expect(mockOnProgress).toHaveBeenCalledWith('NFC kartı algılandı, veriler okunuyor...');
