...
import LivenessDetector, { LIVENESS_STATUS, LIVENESS_INSTRUCTIONS, FaceDetector } from '../modules/liveness/LivenessDetector';
import { getRandomCommand, getCommandByType, generateCommandSequence, getAvailableCommandTypes } from '../modules/liveness/commands';
import { validateResponse, validateRealTimeResponse, validateMockResponse, validateCommandSequence, validateDetectionQuality } from '../modules/liveness/validator';
...
describe('Liveness Detection Tests - Day 9', () => {
  ...
  describe('🎯 Real-time Face Detection', () => {
    ...
    test('should detect head turn motions', async () => {
      ...
      faceDetector.onMotionDetected('lookLeft', () => { leftTurnDetected = true; });
      faceDetector.onMotionDetected('lookRight', () => { rightTurnDetected = true; });
  ...
  describe('🔄 Real-time Integration Tests', () => {
    ...
    test('should handle real-time command execution', async () => {
      ...
      livenessDetector.onInstructionGiven = () => { instructionGiven = true; };
      livenessDetector.onMotionDetected = () => { motionDetected = true; };
  ...
  describe('📷 Camera Functionality (Enhanced Day 8)', () => {
    ...
    test('should call progress callback during camera initialization', async () => {
      ...
      expect(progressCallback).toHaveBeenCalledWith('Ön kamera başlatılıyor...');
      expect(progressCallback).toHaveBeenCalledWith('Kamera hazır. Lütfen kameraya bakın.');
    });
  });
  ...
  describe('📋 Instruction System', () => {
    ...
    test('validateResponse() should return expected result format with mock data', async () => {
      ...
      const result = await livenessDetector.validateResponse(captureData, command);
  ...
  describe('✅ Mock Validation System (Day 8)', () => {
    ...
    test('should handle retry logic with validation failures', async () => {
      ...
      if (!result.isValid && livenessDetector.retryCount < livenessDetector.config.maxRetries) {
        expect(mockCallback).toHaveBeenCalledWith(
  ...
  describe('🔄 Lifecycle Management', () => {
    ...
    test('should prevent concurrent liveness tests', async () => {
      ...
      await expect(
        livenessDetector.startLivenessTest()
      ).rejects.toThrow(/zaten devam ediyor/);
  ...
  describe('📊 Status Management', () => {
    ...
    test('should track captured images count', async () => {
      ...
      await livenessDetector.giveInstruction(LIVENESS_INSTRUCTIONS.LOOK_STRAIGHT);
  ...
  describe('🔧 Error Handling', () => {
    test('should handle camera initialization errors', async () => {
      ...
      livenessDetector.captureFrontCamera = jest.fn().mockRejectedValue(
    ...
    test('should handle instruction errors', async () => {
      await expect(
        livenessDetector.giveInstruction('invalid')
      ).rejects.toThrow('Geçersiz talimat');
    });
    ...
    test('should handle validation errors', async () => {
      ...
      await expect(
        livenessDetector.validateResponse(invalidCaptureData, LIVENESS_INSTRUCTIONS.BLINK)
      ).rejects.toThrow();
    });
  ...
  describe('🧪 Mock Environment (Day 8)', () => {
    test('should check validation environment', async () => {
      const { checkValidationEnvironment } = require('../modules/liveness/validator');
