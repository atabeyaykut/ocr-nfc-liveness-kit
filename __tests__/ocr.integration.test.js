...
describe("OCR Integration Tests", () => {
  ...
  describe("Complete OCR Workflow Tests", () => {
    test("should complete full workflow with Turkish ID card", async () => {
      ...
      expect(result.extractedFields.tcNo).toBe(
        MOCK_ID_CARD_DATA.turkishId.expectedFields.tcNo
      );
      expect(result.extractedFields.name).toBe(
        MOCK_ID_CARD_DATA.turkishId.expectedFields.name
      );
      expect(result.extractedFields.surname).toBe(
        MOCK_ID_CARD_DATA.turkishId.expectedFields.surname
      );
    ...
    test("should handle noisy OCR text and extract fields correctly", async () => {
      ...
      expect(result.extractedFields.tcNo).toBe(
        MOCK_ID_CARD_DATA.noisyText.expectedFields.tcNo
      );
    ...
    test("should handle incomplete ID card data gracefully", async () => {
      ...
      expect(result.extractedFields.tcNo).toBe(
        MOCK_ID_CARD_DATA.incompleteData.expectedFields.tcNo
      );
      expect(result.extractedFields.name).toBe(
        MOCK_ID_CARD_DATA.incompleteData.expectedFields.name
      );
  ...
  describe("Performance Tests", () => {
    test("should complete OCR workflow within acceptable time limits", async () => {
      ...
      expect(result.performanceMetrics.totalProcessingTime).toBeGreaterThan(0);
      expect(result.performanceMetrics.totalProcessingTime).toBeLessThan(
        totalTime
      );
    });
  ...
  describe("Error Handling Tests", () => {
    test("should handle OCR recognition failures gracefully", async () => {
      const { TextRecognition } = require("react-native-text-recognition");
      TextRecognition.recognize.mockRejectedValue(
      ...
      await expect(
        ocrReader.processImageFromWorkflow(imageData)
    ...
    test("should handle image processing failures", async () => {
      ImageProcessor.enhanceImage.mockRejectedValue(
