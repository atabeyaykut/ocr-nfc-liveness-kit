...
public final class MetadataRepo {
    ...
    public static class Node {
        ...
        public Node(int r22) {
            this.mChildren = new SparseArray<>(r22);
        }

        public Node get(int r22) {
            SparseArray<Node> sparseArray = this.mChildren;
            if (sparseArray == null) {
                return null;
            }
            return sparseArray.get(r22);
        }
        ...
        public void put(@NonNull EmojiMetadata emojiMetadata, int r52, int r6) {
            Node node = get(emojiMetadata.getCodepointAt(r52));
            if (node == null) {
                node = new Node();
                this.mChildren.put(emojiMetadata.getCodepointAt(r52), node);
            }
            if (r6 > r52) {
                node.put(emojiMetadata, r52 + 1, r6);
            } else {
    ...
    private void constructIndex(MetadataList metadataList) {
        int r6 = metadataList.listLength();
        for (int r02 = 0; r02 < r6; r02++) {
            EmojiMetadata emojiMetadata = new EmojiMetadata(this, r02);
            Character.toChars(emojiMetadata.getId(), this.mEmojiCharArray, r02 * 2);
            put(emojiMetadata);
        }
    }
