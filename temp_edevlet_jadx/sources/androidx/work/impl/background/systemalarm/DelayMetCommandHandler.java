...
public class DelayMetCommandHandler implements WorkConstraintsCallback, ExecutionListener, WorkTimer.TimeLimitExceededListener {
    ...
    public DelayMetCommandHandler(@NonNull Context context, int r22, @NonNull String str, @NonNull SystemAlarmDispatcher systemAlarmDispatcher) {
    ...
    public void onExecuted(@NonNull String str, boolean z10) {
        Logger.get().debug(TAG, String.format("onExecuted %s, %s", str, Boolean.valueOf(z10)), new Throwable[0]);
        cleanUp();
        if (z10) {
            Intent intentCreateScheduleWorkIntent = CommandHandler.createScheduleWorkIntent(this.mContext, this.mWorkSpecId);
            SystemAlarmDispatcher systemAlarmDispatcher = this.mDispatcher;
