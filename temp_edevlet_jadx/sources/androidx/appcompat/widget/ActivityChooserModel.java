...
class ActivityChooserModel extends DataSetObservable {
    ...
    public static final class DefaultSorter implements ActivitySorter {
        ...
        public void sort(Intent intent, List<ActivityResolveInfo> list, List<HistoricalRecord> list2) {
            ...
            for (int r12 = 0; r12 < size; r12++) {
                ActivityResolveInfo activityResolveInfo = list.get(r12);
    ...
    public static final class HistoricalRecord {
        ...
        public HistoricalRecord(ComponentName componentName, long j10, float f) {
            this.activity = componentName;
            this.time = j10;
            this.weight = f;
        }

        public HistoricalRecord(String str, long j10, float f) {
            this(ComponentName.unflattenFromString(str), j10, f);
        }
        ...
        public int hashCode() {
            ComponentName componentName = this.activity;
            int r02 = componentName == null ? 0 : componentName.hashCode();
            long j10 = this.time;
            return Float.floatToIntBits(this.weight) + ((((r02 + 31) * 31) + ((int) (j10 ^ (j10 >>> 32)))) * 31);
        }
    ...
    private boolean loadActivitiesIfNeeded() {
        ...
        for (int r12 = 0; r12 < size; r12++) {
            this.mActivities.add(new ActivityResolveInfo(listQueryIntentActivities.get(r12)));
        }
        return true;
    }
    ...
    */
    ...
    public Intent chooseActivity(int r72) {
        synchronized (this.mInstanceLock) {
            if (this.mIntent == null) {
                return null;
            }
            ensureConsistentState();
            ActivityInfo activityInfo = this.mActivities.get(r72).resolveInfo.activityInfo;
    ...
    public ResolveInfo getActivity(int r32) {
        ResolveInfo resolveInfo;
        synchronized (this.mInstanceLock) {
            ensureConsistentState();
            resolveInfo = this.mActivities.get(r32).resolveInfo;
        }
        return resolveInfo;
    }
    ...
    public int getActivityIndex(ResolveInfo resolveInfo) {
        synchronized (this.mInstanceLock) {
            ...
            for (int r32 = 0; r32 < size; r32++) {
                if (list.get(r32).resolveInfo == resolveInfo) {
                    return r32;
                }
            }
            return -1;
    ...
    public void setHistoryMaxSize(int r32) {
        synchronized (this.mInstanceLock) {
            if (this.mHistoryMaxSize == r32) {
                return;
            }
            this.mHistoryMaxSize = r32;
            pruneExcessiveHistoricalRecordsIfNeeded();
            if (sortActivitiesIfNeeded()) {
                notifyChanged();
            }
