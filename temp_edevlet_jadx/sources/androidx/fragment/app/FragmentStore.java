...
class FragmentStore {
    ...
    public void dispatchStateChange(int r32) {
        for (FragmentStateManager fragmentStateManager : this.mActive.values()) {
            if (fragmentStateManager != null) {
                fragmentStateManager.setFragmentManagerState(r32);
            }
        }
    }

    public void dump(@NonNull String str, @Nullable FileDescriptor fileDescriptor, @NonNull PrintWriter printWriter, @Nullable String[] strArr) {
        String strF = android.support.v4.media.a.f(str, "    ");
        if (!this.mActive.isEmpty()) {
            ...
            for (FragmentStateManager fragmentStateManager : this.mActive.values()) {
                ...
                if (fragmentStateManager != null) {
                    ...
                    fragment.dump(strF, fileDescriptor, printWriter, strArr);
                } else {
        ...
        if (size > 0) {
            ...
            for (int r72 = 0; r72 < size; r72++) {
                Fragment fragment2 = this.mAdded.get(r72);
                printWriter.print(str);
                printWriter.print("  #");
                printWriter.print(r72);
                printWriter.print(": ");
    ...
    @Nullable
    public Fragment findFragmentById(@IdRes int r42) {
        for (int size = this.mAdded.size() - 1; size >= 0; size--) {
            Fragment fragment = this.mAdded.get(size);
            if (fragment != null && fragment.mFragmentId == r42) {
                return fragment;
            }
        }
        for (FragmentStateManager fragmentStateManager : this.mActive.values()) {
    ...
    public int findFragmentIndexInContainer(@NonNull Fragment fragment) {
        ...
        int r6 = this.mAdded.indexOf(fragment);
        for (int r22 = r6 - 1; r22 >= 0; r22--) {
            Fragment fragment2 = this.mAdded.get(r22);
        ...
        while (true) {
            r6++;
            if (r6 >= this.mAdded.size()) {
                return -1;
            }
            Fragment fragment3 = this.mAdded.get(r6);
        ...
        if (list != null) {
            for (String str : list) {
                ...
                if (fragmentFindActiveFragment == null) {
                    throw new IllegalStateException(androidx.browser.browseractions.a.a("No instantiated fragment for (", str, ")"));
                }
                if (FragmentManager.isLoggingEnabled(2)) {
