...
class TouchResponse {
    ...
    private void fill(TypedArray typedArray) {
        ...
        for (int r22 = 0; r22 < indexCount; r22++) {
            ...
            if (index == R.styleable.OnSwipe_touchAnchorId) {
                ...
                int r32 = typedArray.getInt(index, this.mTouchAnchorSide);
                this.mTouchAnchorSide = r32;
                float[] fArr = TOUCH_SIDES[r32];
                this.mTouchAnchorX = fArr[0];
                this.mTouchAnchorY = fArr[1];
            } else if (index == R.styleable.OnSwipe_dragDirection) {
                int r33 = typedArray.getInt(index, this.mTouchSide);
                this.mTouchSide = r33;
                float[][] fArr2 = TOUCH_DIRECTION;
                if (r33 < fArr2.length) {
                    float[] fArr3 = fArr2[r33];
                    this.mTouchDirectionX = fArr3[0];
                    this.mTouchDirectionY = fArr3[1];
                } else {
                    this.mTouchDirectionY = Float.NaN;
    ...
    public float dot(float f, float f10) {
        return (f10 * this.mTouchDirectionY) + (f * this.mTouchDirectionX);
    }
    ...
    public RectF getLimitBoundsTo(ViewGroup viewGroup, RectF rectF) {
        ...
        int r02 = this.mLimitBoundsTo;
        if (r02 == -1 || (viewFindViewById = viewGroup.findViewById(r02)) == null) {
            return null;
        }
    ...
    public float getProgressDirection(float f, float f10) {
        ...
        return (f10 * this.mTouchDirectionY) / fArr2[1];
    }
    ...
    public void processTouchEvent(MotionEvent motionEvent, MotionLayout.MotionTracker motionTracker, int r23, MotionScene motionScene) {
        int r42;
        if (this.mIsRotateMode) {
            processTouchRotateEvent(motionEvent, motionTracker, r23, motionScene);
            return;
        }
        ...
        if (action == 1) {
            ...
            if (r13 != -1) {
                this.mMotionLayout.getAnchorDpDt(r13, progress, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
            } else {
            ...
            float f10 = !Float.isNaN(fAbs) ? (fAbs / 3.0f) + progress : progress;
            if (f10 != 0.0f && f10 != 1.0f && (r42 = this.mOnTouchUp) != 3) {
                ...
                this.mMotionLayout.touchAnimateTo(this.mOnTouchUp, f11, fAbs);
        ...
        if (Math.abs((this.mTouchDirectionY * rawY) + (this.mTouchDirectionX * rawX)) > this.mDragThreshold || this.mDragStarted) {
            ...
            if (r15 != -1) {
                this.mMotionLayout.getAnchorDpDt(r15, progress2, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
            } else {
    ...
    */
    ...
    public void scrollMove(float f, float f10) {
        float progress = this.mMotionLayout.getProgress();
        if (!this.mDragStarted) {
            this.mDragStarted = true;
        ...
        float f11 = this.mTouchDirectionX;
        float[] fArr = this.mAnchorDpDt;
        if (Math.abs((this.mTouchDirectionY * fArr[1]) + (f11 * fArr[0])) < 0.01d) {
            float[] fArr2 = this.mAnchorDpDt;
            fArr2[0] = 0.01f;
            fArr2[1] = 0.01f;
        }
        float f12 = this.mTouchDirectionX;
        float fMax = Math.max(Math.min(progress + (f12 != 0.0f ? (f * f12) / this.mAnchorDpDt[0] : (f10 * this.mTouchDirectionY) / this.mAnchorDpDt[1]), 1.0f), 0.0f);
        if (fMax != this.mMotionLayout.getProgress()) {
            this.mMotionLayout.setProgress(fMax);
        }
    }

    public void scrollUp(float f, float f10) {
        this.mDragStarted = false;
        float progress = this.mMotionLayout.getProgress();
        this.mMotionLayout.getAnchorDpDt(this.mTouchAnchorId, progress, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
        float f11 = this.mTouchDirectionX;
        float[] fArr = this.mAnchorDpDt;
        float f12 = f11 != 0.0f ? (f * f11) / fArr[0] : (f10 * this.mTouchDirectionY) / fArr[1];
        if (!Float.isNaN(f12)) {
            progress += f12 / 3.0f;
        }
        if (progress != 0.0f) {
            boolean z10 = progress != 1.0f;
            int r32 = this.mOnTouchUp;
            if ((r32 != 3) && z10) {
                this.mMotionLayout.touchAnimateTo(r32, ((double) progress) >= 0.5d ? 1.0f : 0.0f, f12);
            }
        }
    }

    public void setAnchorId(int r12) {
    ...
    public void setDown(float f, float f10) {
        this.mLastTouchX = f;
    ...
    public void setRTL(boolean z10) {
        ...
        int r82 = this.mTouchSide;
        float[][] fArr6 = TOUCH_DIRECTION;
        if (r82 >= fArr6.length) {
            return;
        }
        float[] fArr7 = fArr6[r82];
    ...
    public void setTouchAnchorLocation(float f, float f10) {
        ...
        this.mTouchAnchorY = f10;
    }

    public void setTouchUpMode(int r12) {
        this.mOnTouchUp = r12;
    }

    public void setUpTouchEvent(float f, float f10) {
        this.mLastTouchX = f;
        this.mLastTouchY = f10;
        this.mDragStarted = false;
    }
    ...
    public void setupTouch() {
        View viewFindViewById;
        int r02 = this.mTouchAnchorId;
        if (r02 != -1) {
            viewFindViewById = this.mMotionLayout.findViewById(r02);
        ...
        if (viewFindViewById instanceof NestedScrollView) {
            ...
            nestedScrollView.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener(this) { // from class: androidx.constraintlayout.motion.widget.TouchResponse.2
                ...
                public void onScrollChange(NestedScrollView nestedScrollView2, int r22, int r32, int r42, int r52) {
                }
            });
        }
    }
