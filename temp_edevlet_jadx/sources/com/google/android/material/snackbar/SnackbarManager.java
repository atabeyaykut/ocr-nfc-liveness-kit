...
class SnackbarManager {
    ...
    public static class SnackbarRecord {
        ...
        public SnackbarRecord(int r22, Callback callback) {
    ...
    private boolean cancelSnackbarLocked(@NonNull SnackbarRecord snackbarRecord, int r42) {
        Callback callback = snackbarRecord.callback.get();
        if (callback == null) {
            return false;
        }
        this.handler.removeCallbacksAndMessages(snackbarRecord);
        callback.dismiss(r42);
        return true;
    }
    ...
    private void scheduleTimeoutLocked(@NonNull SnackbarRecord snackbarRecord) {
        int r02 = snackbarRecord.duration;
        if (r02 == -2) {
            return;
        }
        if (r02 <= 0) {
            r02 = r02 == -1 ? SHORT_DURATION_MS : LONG_DURATION_MS;
        }
        this.handler.removeCallbacksAndMessages(snackbarRecord);
        Handler handler = this.handler;
        handler.sendMessageDelayed(Message.obtain(handler, 0, snackbarRecord), r02);
    }
    ...
    public void dismiss(Callback callback, int r42) {
        ...
        synchronized (this.lock) {
            ...
            cancelSnackbarLocked(snackbarRecord, r42);
        }
    }
    ...
    public void show(int r32, Callback callback) {
        synchronized (this.lock) {
            if (isCurrentSnackbarLocked(callback)) {
                SnackbarRecord snackbarRecord = this.currentSnackbar;
                snackbarRecord.duration = r32;
                this.handler.removeCallbacksAndMessages(snackbarRecord);
                scheduleTimeoutLocked(this.currentSnackbar);
                return;
            }
            if (isNextSnackbarLocked(callback)) {
                this.nextSnackbar.duration = r32;
            } else {
                this.nextSnackbar = new SnackbarRecord(r32, callback);
            }
