...
class TimePickerTextInputKeyController implements TextView.OnEditorActionListener, View.OnKeyListener {
    ...
    private void moveSelection(int r52) {
        this.minuteLayoutComboView.setChecked(r52 == 12);
        this.hourLayoutComboView.setChecked(r52 == 10);
        this.time.selection = r52;
    }

    private boolean onHourKeyPress(int r52, KeyEvent keyEvent, EditText editText) {
        Editable text = editText.getText();
        if (text == null) {
            return false;
        }
        if (!(r52 >= 7 && r52 <= 16 && keyEvent.getAction() == 1 && editText.getSelectionStart() == 2 && text.length() == 2)) {
            return false;
        }
        moveSelection(12);
        return true;
    ...
    private boolean onMinuteKeyPress(int r42, KeyEvent keyEvent, EditText editText) {
        if (!(r42 == 67 && keyEvent.getAction() == 0 && TextUtils.isEmpty(editText.getText()))) {
            return false;
        }
        moveSelection(10);
        return true;
    ...
    public boolean onKey(View view, int r52, KeyEvent keyEvent) {
        ...
        boolean zOnMinuteKeyPress = this.time.selection == 12 ? onMinuteKeyPress(r52, keyEvent, editText) : onHourKeyPress(r52, keyEvent, editText);
