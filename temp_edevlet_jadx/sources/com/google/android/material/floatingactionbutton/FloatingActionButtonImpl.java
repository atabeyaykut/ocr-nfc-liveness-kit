...
class FloatingActionButtonImpl {
    ...
    private void calculateImageMatrixFromScale(float f, @NonNull Matrix matrix) {
        ...
        rectF2.set(0.0f, 0.0f, r02, r02);
        matrix.setRectToRect(rectF, rectF2, Matrix.ScaleToFit.CENTER);
        int r03 = this.maxImageSize;
        matrix.postScale(f, f, r03 / 2.0f, r03 / 2.0f);
    }
    ...
    public void getPadding(@NonNull Rect rect) {
        ...
        int r22 = Math.max(sizeDimension, (int) Math.ceil(this.shadowPaddingEnabled ? getElevation() + this.pressedTranslationZ : 0.0f));
        int r02 = Math.max(sizeDimension, (int) Math.ceil(r1 * SHADOW_MULTIPLIER));
        rect.set(r22, r02, r22, r02);
    }
    ...
    public void initializeBackgroundDrawable(ColorStateList colorStateList, @Nullable PorterDuff.Mode mode, ColorStateList colorStateList2, int r42) {
    ...
    public void onDrawableStateChanged(int[] r22) {
        this.stateListAnimator.setState(r22);
    }
    ...
    public void onPaddingUpdated(@NonNull Rect rect) {
        ...
        if (shouldAddPadding()) {
            insetDrawable = new InsetDrawable(this.contentBackground, rect.left, rect.top, rect.right, rect.bottom);
            shadowViewDelegate = this.shadowViewDelegate;
        } else {
            shadowViewDelegate = this.shadowViewDelegate;
            insetDrawable = this.contentBackground;
        }
    ...
    public final void setMaxImageSize(int r22) {
        if (this.maxImageSize != r22) {
            this.maxImageSize = r22;
            updateImageMatrixScale();
        }
    }

    public void setMinTouchTargetSize(int r12) {
        this.minTouchTargetSize = r12;
    }
