...
public class PrivateKeyInfoFactory {
    ...
    public static PrivateKeyInfo createPrivateKeyInfo(AsymmetricKeyParameter asymmetricKeyParameter, ASN1Set aSN1Set) throws IOException {
        ...
        if (parameters2 == null) {
            ...
            if (parameters2 instanceof ECGOST3410Parameters) {
                ...
                byte[] bArr = new byte[ı];
                extractBytes(bArr, ı, 0, eCPrivateKeyParameters.getD());
            ...
            if (!(parameters2 instanceof ECNamedDomainParameters)) {
                ...
                r32 = parameters2.getN().bitLength();
                x962Parameters = x962Parameters2;
                return new PrivateKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_ecPublicKey, x962Parameters), new ECPrivateKey(r32, eCPrivateKeyParameters.getD(), new DERBitString(new FixedPointCombMultiplier().multiply(parameters2.getG(), eCPrivateKeyParameters.getD()).getEncoded(false)), x962Parameters), aSN1Set);
            }
    ...
    private static void extractBytes(byte[] bArr, int r52, int r6, BigInteger bigInteger) {
        byte[] byteArray = bigInteger.toByteArray();
        if (byteArray.length < r52) {
            byte[] bArr2 = new byte[r52];
            System.arraycopy(byteArray, 0, bArr2, r52 - byteArray.length, byteArray.length);
            byteArray = bArr2;
        }
        for (int r12 = 0; r12 != r52; r12++) {
            bArr[r6 + r12] = byteArray[(byteArray.length - 1) - r12];
        }
    }
}
