package org.bouncycastle.crypto.macs;

import org.bouncycastle.crypto.CipherParameters;
import org.bouncycastle.crypto.DataLengthException;
import org.bouncycastle.crypto.Mac;
import org.bouncycastle.crypto.params.KeyParameter;
import org.bouncycastle.crypto.params.ParametersWithIV;

/* loaded from: classes2.dex */
public class VMPCMac implements Mac {
    private byte[] T;

    /* renamed from: g, reason: collision with root package name */
    private byte f11578g;
    private byte[] workingIV;
    private byte[] workingKey;

    /* renamed from: x1, reason: collision with root package name */
    private byte f11581x1;

    /* renamed from: x2, reason: collision with root package name */
    private byte f11582x2;

    /* renamed from: x3, reason: collision with root package name */
    private byte f11583x3;

    /* renamed from: x4, reason: collision with root package name */
    private byte f11584x4;

    /* renamed from: n, reason: collision with root package name */
    private byte f11579n = 0;
    private byte[] P = null;

    /* renamed from: s, reason: collision with root package name */
    private byte f11580s = 0;

    private void initKey(byte[] bArr, byte[] bArr2) {
        this.f11580s = (byte) 0;
        this.P = new byte[256];
        for (int r22 = 0; r22 < 256; r22++) {
            this.P[r22] = (byte) r22;
        }
        for (int r12 = 0; r12 < 768; r12++) {
            byte[] bArr3 = this.P;
            byte b10 = this.f11580s;
            int r42 = r12 & 255;
            byte b11 = bArr3[r42];
            byte b12 = bArr3[(b10 + b11 + bArr[r12 % bArr.length]) & 255];
            this.f11580s = b12;
            bArr3[r42] = bArr3[b12 & 255];
            bArr3[b12 & 255] = b11;
        }
        for (int r82 = 0; r82 < 768; r82++) {
            byte[] bArr4 = this.P;
            byte b13 = this.f11580s;
            int r43 = r82 & 255;
            byte b14 = bArr4[r43];
            byte b15 = bArr4[(b13 + b14 + bArr2[r82 % bArr2.length]) & 255];
            this.f11580s = b15;
            bArr4[r43] = bArr4[b15 & 255];
            bArr4[b15 & 255] = b14;
        }
        this.f11579n = (byte) 0;
    }

    @Override // org.bouncycastle.crypto.Mac
    public int doFinal(byte[] bArr, int r15) throws IllegalStateException, DataLengthException {
        for (int r12 = 1; r12 < 25; r12++) {
            byte[] bArr2 = this.P;
            byte b10 = this.f11580s;
            byte b11 = this.f11579n;
            byte b12 = bArr2[(b10 + bArr2[b11 & 255]) & 255];
            this.f11580s = b12;
            byte b13 = this.f11584x4;
            byte b14 = this.f11583x3;
            byte b15 = bArr2[(b13 + b14 + r12) & 255];
            this.f11584x4 = b15;
            byte b16 = this.f11582x2;
            byte b17 = bArr2[(b14 + b16 + r12) & 255];
            this.f11583x3 = b17;
            byte b18 = this.f11581x1;
            byte b19 = bArr2[(b16 + b18 + r12) & 255];
            this.f11582x2 = b19;
            byte b20 = bArr2[(b18 + b12 + r12) & 255];
            this.f11581x1 = b20;
            byte[] bArr3 = this.T;
            byte b21 = this.f11578g;
            bArr3[b21 & 31] = (byte) (b20 ^ bArr3[b21 & 31]);
            bArr3[(b21 + 1) & 31] = (byte) (b19 ^ bArr3[(b21 + 1) & 31]);
            bArr3[(b21 + 2) & 31] = (byte) (b17 ^ bArr3[(b21 + 2) & 31]);
            bArr3[(b21 + 3) & 31] = (byte) (b15 ^ bArr3[(b21 + 3) & 31]);
            this.f11578g = (byte) ((b21 + 4) & 31);
            byte b22 = bArr2[b11 & 255];
            bArr2[b11 & 255] = bArr2[b12 & 255];
            bArr2[b12 & 255] = b22;
            this.f11579n = (byte) ((b11 + 1) & 255);
        }
        for (int r22 = 0; r22 < 768; r22++) {
            byte[] bArr4 = this.P;
            byte b23 = this.f11580s;
            int r52 = r22 & 255;
            byte b24 = bArr4[r52];
            byte b25 = bArr4[(b23 + b24 + this.T[r22 & 31]) & 255];
            this.f11580s = b25;
            bArr4[r52] = bArr4[b25 & 255];
            bArr4[b25 & 255] = b24;
        }
        byte[] bArr5 = new byte[20];
        for (int r42 = 0; r42 < 20; r42++) {
            byte[] bArr6 = this.P;
            int r72 = r42 & 255;
            byte b26 = bArr6[(this.f11580s + bArr6[r72]) & 255];
            this.f11580s = b26;
            bArr5[r42] = bArr6[(bArr6[bArr6[b26 & 255] & 255] + 1) & 255];
            byte b27 = bArr6[r72];
            bArr6[r72] = bArr6[b26 & 255];
            bArr6[b26 & 255] = b27;
        }
        System.arraycopy(bArr5, 0, bArr, r15, 20);
        reset();
        return 20;
    }

    @Override // org.bouncycastle.crypto.Mac
    public String getAlgorithmName() {
        return "VMPC-MAC";
    }

    @Override // org.bouncycastle.crypto.Mac
    public int getMacSize() {
        return 20;
    }

    @Override // org.bouncycastle.crypto.Mac
    public void init(CipherParameters cipherParameters) throws IllegalArgumentException {
        if (!(cipherParameters instanceof ParametersWithIV)) {
            throw new IllegalArgumentException("VMPC-MAC Init parameters must include an IV");
        }
        ParametersWithIV parametersWithIV = (ParametersWithIV) cipherParameters;
        KeyParameter keyParameter = (KeyParameter) parametersWithIV.getParameters();
        if (!(parametersWithIV.getParameters() instanceof KeyParameter)) {
            throw new IllegalArgumentException("VMPC-MAC Init parameters must include a key");
        }
        byte[] bArr = parametersWithIV.getIV();
        this.workingIV = bArr;
        if (bArr == null || bArr.length < 1 || bArr.length > 768) {
            throw new IllegalArgumentException("VMPC-MAC requires 1 to 768 bytes of IV");
        }
        this.workingKey = keyParameter.getKey();
        reset();
    }

    @Override // org.bouncycastle.crypto.Mac
    public void reset() {
        initKey(this.workingKey, this.workingIV);
        this.f11579n = (byte) 0;
        this.f11584x4 = (byte) 0;
        this.f11583x3 = (byte) 0;
        this.f11582x2 = (byte) 0;
        this.f11581x1 = (byte) 0;
        this.f11578g = (byte) 0;
        this.T = new byte[32];
        for (int r22 = 0; r22 < 32; r22++) {
            this.T[r22] = 0;
        }
    }

    @Override // org.bouncycastle.crypto.Mac
    public void update(byte b10) throws IllegalStateException {
        byte[] bArr = this.P;
        byte b11 = this.f11580s;
        byte b12 = this.f11579n;
        byte b13 = bArr[(b11 + bArr[b12 & 255]) & 255];
        this.f11580s = b13;
        byte b14 = (byte) (b10 ^ bArr[(bArr[bArr[b13 & 255] & 255] + 1) & 255]);
        byte b15 = this.f11584x4;
        byte b16 = this.f11583x3;
        byte b17 = bArr[(b15 + b16) & 255];
        this.f11584x4 = b17;
        byte b18 = this.f11582x2;
        byte b19 = bArr[(b16 + b18) & 255];
        this.f11583x3 = b19;
        byte b20 = this.f11581x1;
        byte b21 = bArr[(b18 + b20) & 255];
        this.f11582x2 = b21;
        byte b22 = bArr[(b20 + b13 + b14) & 255];
        this.f11581x1 = b22;
        byte[] bArr2 = this.T;
        byte b23 = this.f11578g;
        bArr2[b23 & 31] = (byte) (b22 ^ bArr2[b23 & 31]);
        bArr2[(b23 + 1) & 31] = (byte) (b21 ^ bArr2[(b23 + 1) & 31]);
        bArr2[(b23 + 2) & 31] = (byte) (b19 ^ bArr2[(b23 + 2) & 31]);
        bArr2[(b23 + 3) & 31] = (byte) (b17 ^ bArr2[(b23 + 3) & 31]);
        this.f11578g = (byte) ((b23 + 4) & 31);
        byte b24 = bArr[b12 & 255];
        bArr[b12 & 255] = bArr[b13 & 255];
        bArr[b13 & 255] = b24;
        this.f11579n = (byte) ((b12 + 1) & 255);
    }

    @Override // org.bouncycastle.crypto.Mac
    public void update(byte[] bArr, int r42, int r52) throws IllegalStateException, DataLengthException {
        if (r42 + r52 > bArr.length) {
            throw new DataLengthException("input buffer too short");
        }
        for (int r02 = 0; r02 < r52; r02++) {
            update(bArr[r42 + r02]);
        }
    }
}
