        ...
        if ((!this.targetNames.isEmpty() || !this.targetGroups.isEmpty()) && (extensionValue = x509AttributeCertificate.getExtensionValue(Extension.targetInformation.getId())) != null) {
            try {
                ...
                if (!this.targetNames.isEmpty()) {
                    ...
                    for (Targets targets : targetsObjects) {
                        ...
                        int r52 = 0;
                        while (true) {
                            if (r52 >= targets2.length) {
                                break;
                            }
                            if (this.targetNames.contains(GeneralName.getInstance(targets2[r52].getTargetName()))) {
                                z10 = true;
                                break;
                            }
                            r52++;
            ...
            if (!this.targetGroups.isEmpty()) {
                ...
                for (Targets targets3 : targetsObjects) {
                    ...
                    int r53 = 0;
                    while (true) {
                        if (r53 >= targets4.length) {
                            break;
                        }
                        if (this.targetGroups.contains(GeneralName.getInstance(targets4[r53].getTargetGroup()))) {
                            z11 = true;
                            break;
                        }
                        r53++;
