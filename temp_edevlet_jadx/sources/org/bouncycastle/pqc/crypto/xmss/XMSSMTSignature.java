...
public final class XMSSMTSignature implements XMSSStoreableObjectInterface, Encodable {
    ...
    private XMSSMTSignature(Builder builder) {
        ...
        int r32 = (int) Math.ceil(xMSSMTParameters.getHeight() / 8.0d);
        int height = ((xMSSMTParameters.getHeight() / xMSSMTParameters.getLayers()) + len) * treeDigestSize;
        if (bArr.length != r32 + treeDigestSize + (xMSSMTParameters.getLayers() * height)) {
            throw new IllegalArgumentException("signature has wrong size");
        }
        long jBytesToXBigEndian = XMSSUtil.bytesToXBigEndian(bArr, 0, r32);
        this.index = jBytesToXBigEndian;
        if (!XMSSUtil.isIndexValid(xMSSMTParameters.getHeight(), jBytesToXBigEndian)) {
            throw new IllegalArgumentException("index out of bounds");
        }
        int r33 = r32 + 0;
        this.random = XMSSUtil.extractBytesAtOffset(bArr, r33, treeDigestSize);
        this.reducedSignatures = new ArrayList();
        for (int r34 = r33 + treeDigestSize; r34 < bArr.length; r34 += height) {
            this.reducedSignatures.add(new XMSSReducedSignature.Builder(this.params.getXMSSParameters()).withReducedSignature(XMSSUtil.extractBytesAtOffset(bArr, r34, height)).build());
        }
    }
    ...
    public byte[] toByteArray() {
        ...
        int r22 = (int) Math.ceil(this.params.getHeight() / 8.0d);
        int height = ((this.params.getHeight() / this.params.getLayers()) + len) * treeDigestSize;
        byte[] bArr = new byte[r22 + treeDigestSize + (this.params.getLayers() * height)];
        XMSSUtil.copyBytesAtOffset(bArr, XMSSUtil.toBytesBigEndian(this.index, r22), 0);
        int r23 = r22 + 0;
        XMSSUtil.copyBytesAtOffset(bArr, this.random, r23);
        int r24 = r23 + treeDigestSize;
        Iterator<XMSSReducedSignature> it = this.reducedSignatures.iterator();
        while (it.hasNext()) {
            XMSSUtil.copyBytesAtOffset(bArr, it.next().toByteArray(), r24);
            r24 += height;
        }
        return bArr;
    }
