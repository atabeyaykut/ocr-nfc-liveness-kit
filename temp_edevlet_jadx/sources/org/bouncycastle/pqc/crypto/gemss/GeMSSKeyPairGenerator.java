...
public class GeMSSKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
    ...
    public AsymmetricCipherKeyPair generateKeyPair() {
        ...
        int r22 = engine.HFEDegJ + 2;
        int r32 = engine.HFEDegI;
        int r33 = (((r32 + 1) * engine.HFEv) + (engine.NB_MONOMIAL_VINEGAR - 1) + r22 + (((r32 + 1) * r32) >>> 1)) * engine.NB_WORD_GFqn;
        int r23 = (((engine.LTRIANGULAR_NV_SIZE << 1) + r33) + (engine.LTRIANGULAR_N_SIZE << 1)) << 3;
        Pointer pointer = new Pointer(r23 >>> 3);
        byte[] bArr = new byte[r23];
        SHAKEDigest sHAKEDigest = new SHAKEDigest(engine.ShakeBitStrength);
        int r92 = 0;
        sHAKEDigest.update(bArrSec_rand, 0, engine.SIZE_SEED_SK);
        sHAKEDigest.doFinal(bArr, 0, r23);
        int r72 = engine.SIZE_SEED_SK;
        byte[] bArr2 = new byte[r72];
        byte[] bArr3 = new byte[((engine.NB_MONOMIAL_PK * engine.HFEm) + 7) >> 3];
        System.arraycopy(bArrSec_rand, 0, bArr2, 0, r72);
        pointer.fill(0, bArr, 0, r23);
        ...
        Pointer pointer5 = new Pointer(pointer, r33);
        ...
        if (engine.HFEmr8 != 0) {
            int r24 = engine.NB_MONOMIAL_PK;
            int r34 = engine.NB_BYTES_GFqm;
            PointerUnion pointerUnion = new PointerUnion((r24 * r34) + ((8 - (r34 & 7)) & 7));
            for (int r25 = (engine.NB_BYTES_GFqm & 7) != 0 ? 1 : 0; r25 < engine.NB_MONOMIAL_PK; r25++) {
            ...
            if ((engine.NB_BYTES_GFqm & 7) != 0) {
                ...
                while (r92 < engine.NB_WORD_GF2m) {
                    pointerUnion.set(r92, pointer7.get(r92));
                    r92++;
                }
            }
            ...
            int bytesMove = 0;
            while (r92 < engine.NB_MONOMIAL_PK) {
