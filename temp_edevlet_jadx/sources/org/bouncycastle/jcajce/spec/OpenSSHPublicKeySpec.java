...
public class OpenSSHPublicKeySpec extends EncodedKeySpec {
    ...
    public OpenSSHPublicKeySpec(byte[] bArr) {
        super(bArr);
        int r02 = 0;
        int r12 = (((bArr[0] & 255) << 24) | ((bArr[1] & 255) << 16) | ((bArr[2] & 255) << 8) | (bArr[3] & 255)) + 4;
        if (r12 >= bArr.length) {
            throw new IllegalArgumentException("invalid public key blob: type field longer than blob");
        }
        String strFromByteArray = Strings.fromByteArray(Arrays.copyOfRange(bArr, 4, r12));
        ...
        while (true) {
            ...
            if (strArr[r02].equals(this.type)) {
                return;
            } else {
                r02++;
            }
