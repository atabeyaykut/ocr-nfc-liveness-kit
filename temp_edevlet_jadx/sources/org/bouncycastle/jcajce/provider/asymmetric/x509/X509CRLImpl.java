...
abstract class X509CRLImpl extends X509CRL {
    ...
    protected CertificateList f11674c;
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, CRLException, InvalidAlgorithmParameterException {
        if (!X509SignatureUtil.areEquivalentAlgorithms(this.f11674c.getSignatureAlgorithm(), this.f11674c.getTBSCertList().getSignature())) {
        ...
        try {
            ...
            this.f11674c.getTBSCertList().encodeTo(bufferedOutputStream, ASN1Encoding.DER);
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, CRLException, NoSuchProviderException, InvalidAlgorithmParameterException {
        if (!this.f11674c.getSignatureAlgorithm().equals(this.f11674c.getTBSCertList().getSignature())) {
            throw new CRLException("Signature algorithm on CertificateList does not match TBSCertList.");
        }
        int r32 = 0;
        if ((publicKey instanceof CompositePublicKey) && X509SignatureUtil.isCompositeAlgorithm(this.f11674c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence aSN1Sequence = ASN1Sequence.getInstance(this.f11674c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence2 = ASN1Sequence.getInstance(this.f11674c.getSignature().getOctets());
            boolean z10 = false;
            while (r32 != publicKeys.size()) {
                if (publicKeys.get(r32) != null) {
                    AlgorithmIdentifier algorithmIdentifier = AlgorithmIdentifier.getInstance(aSN1Sequence.getObjectAt(r32));
                    try {
                        checkSignature(publicKeys.get(r32), signatureCreator.createSignature(X509SignatureUtil.getSignatureName(algorithmIdentifier)), algorithmIdentifier.getParameters(), ASN1BitString.getInstance(aSN1Sequence2.getObjectAt(r32)).getOctets());
                        z10 = true;
                        e = null;
        ...
        if (!X509SignatureUtil.isCompositeAlgorithm(this.f11674c.getSignatureAlgorithm())) {
            ...
            try {
                ...
                throw new SignatureException(a.c(e11, new StringBuilder("cannot decode signature parameters: ")));
            }
        }
        ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(this.f11674c.getSignatureAlgorithm().getParameters());
        ASN1Sequence aSN1Sequence4 = ASN1Sequence.getInstance(this.f11674c.getSignature().getOctets());
        boolean z11 = false;
        while (r32 != aSN1Sequence4.size()) {
            AlgorithmIdentifier algorithmIdentifier2 = AlgorithmIdentifier.getInstance(aSN1Sequence3.getObjectAt(r32));
            try {
                checkSignature(publicKey, signatureCreator.createSignature(X509SignatureUtil.getSignatureName(algorithmIdentifier2)), algorithmIdentifier2.getParameters(), ASN1BitString.getInstance(aSN1Sequence4.getObjectAt(r32)).getOctets());
                z11 = true;
    ...
    private Set getExtensionOIDs(boolean z10) {
        ...
        if (getVersion() != 2 || (extensions = this.f11674c.getTBSCertList().getExtensions()) == null) {
            return null;
        }
    ...
    private Set loadCRLEntries() {
        Extension extension;
        HashSet hashSet = new HashSet();
        Enumeration revokedCertificateEnumeration = this.f11674c.getRevokedCertificateEnumeration();
    ...
    public byte[] getExtensionValue(String str) {
        ASN1OctetString extensionValue = getExtensionValue(this.f11674c, str);
        if (extensionValue == null) {
            return null;
        }
        try {
    ...
    public Principal getIssuerDN() {
        return new X509Principal(X500Name.getInstance(this.f11674c.getIssuer().toASN1Primitive()));
    }

    @Override // java.security.cert.X509CRL
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f11674c.getIssuer().getEncoded());
    ...
    public Date getNextUpdate() {
        Time nextUpdate = this.f11674c.getNextUpdate();
        if (nextUpdate == null) {
            return null;
        }
    ...
    public String getSigAlgOID() {
        return this.f11674c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f11674c.getSignature().getOctets();
    }

    @Override // java.security.cert.X509CRL
    public byte[] getTBSCertList() throws CRLException {
        try {
            return this.f11674c.getTBSCertList().getEncoded(ASN1Encoding.DER);
        } catch (IOException e10) {
            throw new CRLException(e10.toString());
        }
    }
    ...
    public Date getThisUpdate() {
        return this.f11674c.getThisUpdate().getDate();
    }

    @Override // java.security.cert.X509CRL
    public int getVersion() {
        return this.f11674c.getVersionNumber();
    }
    ...
    public boolean isRevoked(Certificate certificate) {
        ...
        Enumeration revokedCertificateEnumeration = this.f11674c.getRevokedCertificateEnumeration();
        X500Name issuer2 = this.f11674c.getIssuer();
