...
const LivenessDemoScreen = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'camera_ready': return '#2196F3';
      case 'instruction_given': return '#9C27B0';
      case 'capturing': return '#FF5722';
  ...
  const simulateSpoofingTest = async () => {
    try {
      ...
      addLog(`🔍 Güven skoru: ${(simulatedResult.confidence * 100).toFixed(1)}%`, 'info');
      
      // Show alert
      Alert.alert(
        '❌ Sahte Tespit Edildi',
        `Sistem sahte bir giriş tespit etti.\n\nSebep: ${simulatedResult.reason}\nGüven Skoru: ${(simulatedResult.confidence * 100).toFixed(1)}%`,
        [{ text: 'Tamam' }]
      );
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            ...
            disabled={isProcessing || (realTimeMode && cameraPermission !== 'authorized')}
          >
            {isProcessing ? (
        ...
        {antiSpoofingResult && (
          <View style={[styles.resultContainer, 
            antiSpoofingResult.isReal ? styles.realResultContainer : styles.fakeResultContainer
          ]}>
            <Text style={styles.resultTitle}>
              {antiSpoofingResult.isReal ? '✅ Gerçek Yüz Tespit Edildi' : '❌ Sahte Tespit Edildi'}
            </Text>
        ...
        {testResults && (
          <View style={styles.resultContainer}>
            ...
            {testResults.antiSpoofingEnabled && (
              <Text style={styles.resultText}>
                Anti-Spoofing: {testResults.antiSpoofingResult?.isReal ? '✅ Geçti' : '❌ Başarısız'}
              </Text>
            )}
          </View>
        )}
